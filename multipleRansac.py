...
def superimpose_all_inliers(self,ransac_lines,width:float, height:float):
...
def extract_multiple_lines_and_save(inputfilename:str,iterations:int, max_distance:int,min_inliers_allowed:int):
    ...
    absolute_path=os.path.join(folder_script,"photo/",inputfilename)
    ...
    #drawLines(absolute_path,results,color=(0,0,255))
    #Save the results
    filename_noextension=os.path.splitext(inputfilename)[0]
    folder_script=os.path.dirname(__file__)
    file_result=os.path.join(folder_script,"./out/",("test1-%s.png") % (filename_noextension))
...
def drawLines(img, ransac_lines, color=(0,0,255)):
    """
...
extract_multiple_lines_and_save("test2.png",iterations= 30,max_distance=1, min_inliers_allowed=50)
