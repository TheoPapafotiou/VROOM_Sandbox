...
class SignDetection:
    ...
    def __init__(self):
        ...
        self.classes = ['ParkingSpot', 'Crosswalk', 'Ahead', 'HighwayEnd', 'HighwayStart', 'PriorityRoad', 'Stop',
                        'NoEntry', 'Roundabout', 'TrafficLights', 'Roadblock', 'Vehicle']
    ...
    def detectSignProcedure(self, blob, img, height, width):
        ...
        if len(indexes) > 0:
            for i in indexes.flatten():
                ...
                cv2.putText(img, self.label, (x - int(self.x_camera / 2), y + int(self.y_camera / 4)), font, 2, color,
                            2)
                cv2.putText(img, "Confidence: " + self.confidence,
                            (x - int(self.x_camera / 2), y + int(self.y_camera / 3)), font, 2, color, 2)
